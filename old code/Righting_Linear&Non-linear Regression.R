...
linear <- ggplot(night_dd, aes(pitch_initial, rot_l_decel, col = "red"))+
...
binomial <- ggplot(night_dd, aes(pitch_initial,rot_l_decel, col = "red"))+
  geom_point(alpha = 0.01)+
...
trinomial <- ggplot(night_dd, aes(pitch_initial,rot_l_decel,  col = "red"))+
  geom_point(alpha = 0.01)+
...
#########################
# Export x, y 
y = night_dd$pitch_initial 
x = night_dd$rot_l_decel
...
x_predict = seq(-10, 20, length.out = length(night_dd$pitch_initial))
...
# Vertical plot
logistic_3par <- ggplot(night_dd, aes(pitch_initial, rot_l_decel, col ="red"))+
  geom_point(alpha = 0.01)+
...
summary(fit2)
x_predict_fit2 = seq(-10, 20, length.out = length(night_dd$pitch_initial))
...
loess<-ggplot(night_dd, aes(pitch_initial,rot_l_decel, col = "red"))+
  geom_point(alpha = 0.01)+
  stat_smooth(color = "blue")+
  ggtitle("Night time DD (LOESS Regression, no equation)")

plot_grid(logistic_3par, logistic_4par, loess, 
          ncol = 3, nrow = 1)
