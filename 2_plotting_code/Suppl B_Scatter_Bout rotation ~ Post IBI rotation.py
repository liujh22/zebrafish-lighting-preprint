"""
Script: Plot Suppl B rot_total vs postIBI_rot relationship
Description: Analyzes and visualizes the relationship between rotation and post-IBI rotation,
             stratified by IBI duration groups in wild-type data, using robust regression.
"""
#%%
import os
...
def main():
    ...
    dd_data = combine_data[combine_data['cond1'] == "dd"].copy()

    # --------------------------------------------------------------------------
    # Create IBI Duration Groups
    # --------------------------------------------------------------------------
    median_post_ibi = dd_data['post_IBI_time'].quantile(0.5)
    dd_data['IBI_group'] = np.where(
        dd_data['post_IBI_time'] >= median_post_ibi,
        "Long IBI",
        "Short IBI"
    )
    dd_data['IBI_group'] = pd.Categorical(
        dd_data['IBI_group'],
    ...
    for group_name, group_df in dd_data.groupby("IBI_group"):
    ...
    plot_data = dd_data.sample(min(SAMPLE_SIZE, len(dd_data)), random_state=42)

    plot = (
            ggplot(plot_data, aes(x=X_VAR, y=Y_VAR, color='IBI_group'))
    ...
    # Set axis limits
    x_lower, x_upper = dd_data[X_VAR].quantile(0.005), dd_data[X_VAR].quantile(0.995)
    y_lower, y_upper = dd_data[Y_VAR].quantile(0.005), dd_data[Y_VAR].quantile(0.995)
    ...
    output_filename = f"{plot_ID}_scatter_{Y_VAR}_by_{X_VAR}.pdf"
    ...
    # --------------------------------------------------------------------------
    print("\nDark Post-IBI: Slope, Rsquare and p-value Table:")
    ...
    macros_path = os.path.join(STATS_DIR, f"{plot_ID}_GroupedScatterBoutRotationPostIBIMacros.tex")

    with open(macros_path, "w") as tex_file:
        for row in stats_results:
            grp = row['IBI_group'].replace(" ", "")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPostIBI{grp}AbsSlope{{{abs(row['Slope']):.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPostIBI{grp}Slope{{{row['Slope']:.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPostIBI{grp}Rsquare{{{row['Rsquare']:.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPostIBI{grp}Pvalue{{{row['pvalue']:.3e}}}\n")
