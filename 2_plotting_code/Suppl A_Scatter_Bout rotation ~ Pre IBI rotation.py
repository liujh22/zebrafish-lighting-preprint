"""
Script: Plot Suppl A rot_total vs preIBI_rot relationship
...
def main():
    ...
    # --------------------------------------------------------------------------
    median_pre_ibi = dd_data['pre_IBI_time'].quantile(0.5)
    ...
    macros_path = os.path.join(STATS_DIR, f"{plot_ID}_GroupedScatterBoutRotationPreIBIMacros.tex")

    with open(macros_path, "w") as tex_file:
        for row in stats_results:
            grp = row['IBI_group'].replace(" ", "")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPreIBI{grp}AbsSlope{{{abs(row['Slope']):.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPreIBI{grp}Slope{{{row['Slope']:.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPreIBI{grp}Rsquare{{{row['Rsquare']:.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterBoutRotationPreIBI{grp}Pvalue{{{row['pvalue']:.3e}}}\n")
