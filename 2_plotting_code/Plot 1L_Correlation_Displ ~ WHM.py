"""
Correlation displ_swim vs WHM
Description: Computes and visualizes relationship between displacement and bout duration with binned medians and IQR.
"""
#%%
import os
...
def main():
    ...
    # Clip WHM values
    whm_lower, whm_upper = raw_data["WHM"].quantile(WHM_CLIP).values
    combine_data = combine_data[
        (combine_data["WHM"] >= whm_lower) &
        (combine_data["WHM"] <= whm_upper)
        ]
    ...
    plot = (
            ...
            + labs(x="bout duration (ms)",
                   y="Displacement (mm)",
                   title="Displacement vs bout duration")
            + theme_minimal()
            + theme(
