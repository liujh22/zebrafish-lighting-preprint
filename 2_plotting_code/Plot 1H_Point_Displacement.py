...
def print_results(t1, p1, t2, p2):
    ...
    print("—— Independent samples t-test (epoch-level displ_swim, dd vs ld) ——")
    print(f"t = {t1:.3f}, p = {p1:.3e}")
    print("\n—— Independent samples t-test (expNum-median displ_swim, dd vs ld) ——")
...
def summary_stats(median_data, t_value, p_value, stats_dir):
    ...
    macros_path = os.path.join(stats_dir, f"{plot_ID}_GroupedDisplSwimMacros.tex")

    macros = [
        f"\\def\\GroupedDisplSwimDdDayMean{{{dd_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\GroupedDisplSwimDdDaySD{{{dd_stats['SD'].values[0]:.3f}}}",
        f"\\def\\GroupedDisplSwimLdDayMean{{{ld_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\GroupedDisplSwimLdDaySD{{{ld_stats['SD'].values[0]:.3f}}}",
        f"\\def\\GroupedDisplSwimDdDayVsLdDayTvalue{{{t_value:.3f}}}",
        f"\\def\\GroupedDisplSwimDdDayVsLdDayPvalue{{{p_value:.3e}}}",
        f"\\def\\GroupedDisplSwimDdDayVsLdDayEffectSize{{{abs(effect_size):.3f}}}"
    ]
...
def main():
    ...
    # Take absolute value of displacement
    combine_data[Y_VAR] = combine_data[Y_VAR].abs()
