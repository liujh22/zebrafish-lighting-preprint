"""
Description: Computes and visualizes median WHM values per bout under different lighting conditions.
"""
#%%
import os
...
def print_results(t1, p1, t2, p2):
    ...
    print("—— Independent samples t-test (epoch-level WHM, dd vs ld) ——")
    print(f"t = {t1:.3f}, p = {p1:.3e}")
    print("\n—— Independent samples t-test (expNum-median WHM, dd vs ld) ——")
...
def summary_stats(median_data, t_value, p_value, stats_dir):
    ...
    macros_path = os.path.join(stats_dir, f"{plot_ID}_GroupedWHMMacros.tex")

    macros = [
        f"\\def\\GroupedWHMDdDayMean{{{dd_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\GroupedWHMDdDaySD{{{dd_stats['SD'].values[0]:.3f}}}",
        f"\\def\\GroupedWHMLdDayMean{{{ld_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\GroupedWHMLdDaySD{{{ld_stats['SD'].values[0]:.3f}}}",
        f"\\def\\GroupedWHMDdDayVsLdDayTvalue{{{t_value:.3f}}}",
        f"\\def\\GroupedWHMDdDayVsLdDayPvalue{{{p_value:.3e}}}",
        f"\\def\\GroupedWHMDdDayVsLdDayEffectSize{{{abs(effect_size):.3f}}}"
    ]
...
def main():
    ...
    whm_dd = combine_data.loc[combine_data[X_VAR] == "dd", Y_VAR].dropna()
    whm_ld = combine_data.loc[combine_data[X_VAR] == "ld", Y_VAR].dropna()
    t1, p1 = stats.ttest_ind(whm_dd, whm_ld, nan_policy='omit')
