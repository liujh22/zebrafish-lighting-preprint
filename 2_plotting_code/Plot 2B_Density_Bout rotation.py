...
def main():
    ...
    plot = (
        ...
        + geom_line(size=1)
        + labs(x="Total Rotation (deg)",
               y="Normalized Frequency",
               title="Rotation Distribution by Condition")
        + theme_minimal()
    ...
    dd_rot = combine_data.loc[combine_data[X_VAR] == "dd", Y_VAR].dropna()
    ld_rot = combine_data.loc[combine_data[X_VAR] == "ld", Y_VAR].dropna()

    ks_stat, ks_p = stats.ks_2samp(dd_rot, ld_rot)
