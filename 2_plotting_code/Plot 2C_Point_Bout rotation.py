"""
Median rot_total by Condition
Description: Computes and visualizes median rot_total per bout under different lighting conditions.
"""
#%%
import os
...
def summary_stats(median_data, t_value, p_value, stats_dir, y_var):
    ...
    latex_var = ''.join(word.capitalize() for word in y_var.split('_'))
    ...
    macros = [
        f"\\def\\Grouped{latex_var}DdDayMean{{{dd_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\Grouped{latex_var}DdDaySD{{{dd_stats['SD'].values[0]:.3f}}}",
        f"\\def\\Grouped{latex_var}LdDayMean{{{ld_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\Grouped{latex_var}LdDaySD{{{ld_stats['SD'].values[0]:.3f}}}",
        f"\\def\\Grouped{latex_var}DdDayVsLdDayTvalue{{{t_value:.3f}}}",
        f"\\def\\Grouped{latex_var}DdDayVsLdDayPvalue{{{p_value:.3e}}}",
        f"\\def\\Grouped{latex_var}DdDayVsLdDayEffectSize{{{abs(effect_size):.3f}}}"
    ]
...
def main():
    ...
    plot = (
        ...
        + labs(x="Condition (dd vs ld)",
               y="Median rot_total",
               title="Median rot_total by Condition")
        + theme_minimal()
        + theme(
    ...
    summary_stats(median_data, t2, p2, STATS_DIR, Y_VAR)
    #%%
