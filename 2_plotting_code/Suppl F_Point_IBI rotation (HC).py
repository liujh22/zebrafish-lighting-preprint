"""
Script: Plot Suppl F Median preIBI_rot
Description: Computes and visualizes median preIBI_rot per bout under different experimental conditions.
"""
#%%
...
def main():
    ...
    # rot_ctrl = combine_data.loc[combine_data[X_VAR] == "1ctrl", Y_VAR].dropna()
    # rot_cond = combine_data.loc[combine_data[X_VAR] == "2cond", Y_VAR].dropna()
    # t1, p1 = stats.ttest_ind(rot_ctrl, rot_cond, nan_policy='omit')
    ...
    # Print results
    # print("—— Independent samples t-test (epoch-level preIBI_rot, ctrl vs cond) ——")
    # print(f"t = {t1:.3f}, p = {p1:.3e}")
    print("\n—— Independent samples t-test (expNum-median preIBI_rot, ctrl vs cond) ——")
    ...
    macros_path = os.path.join(STATS_DIR, f"{plot_ID}_GroupedPreIBIrotMacros.tex")

    with open(macros_path, "w") as f:
        f.write(f"\\def\\GroupedPreIBIrotCtrlMean{{{ctrl_stats['Mean'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedPreIBIrotCtrlSD{{{ctrl_stats['SD'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedPreIBIrotCondMean{{{cond_stats['Mean'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedPreIBIrotCondSD{{{cond_stats['SD'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedPreIBIrotCtrlVsCondTvalue{{{t2:.3f}}}\n")
        f.write(f"\\def\\GroupedPreIBIrotCtrlVsCondPvalue{{{p2:.3e}}}\n")
        f.write(f"\\def\\GroupedPreIBIrotCtrlVsCondEffectSize{{{abs(effect_size):.3f}}}\n")
