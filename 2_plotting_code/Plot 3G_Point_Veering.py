...
from plotnine import (
    ggplot, aes, geom_segment, geom_errorbar, geom_point, labs,
    scale_y_continuous, scale_x_continuous, theme_minimal, coord_fixed,xlim,
...
def summarise_epoch(grp):
    ...
    # Compute veering_total: sum of absolute differences of traj_peak across the first 4 bouts
...
p = (ggplot(median_data, aes(x='cond1', y='median_value', color='cond1'))
     ...
     # + labs(x="Condition", y="Veering (1s-normalized)")
     + theme_minimal()
     + theme(
         axis_ticks_minor = element_blank(),      # Remove minor tick marks
         axis_line=element_line(color="black"),   # Set axis line color to black
         axis_ticks=element_line(color="black"),  # Set major tick mark color to black
         panel_grid=element_blank()               # Remove panel grid lines
     )
)
...
# 1) Independent samples t-test (treat each epoch's 1s_veering as an independent observation)
veering_dd = df_1s.loc[df_1s['cond1']=="dd", '1s_veering'].dropna()
veering_ld = df_1s.loc[df_1s['cond1']=="ld", '1s_veering'].dropna()

t_ind, p_ind = stats.ttest_ind(veering_dd, veering_ld, nan_policy='omit')
print("—— Independent samples t-test (epoch-level 1s_veering, dd vs ld) ——")
...
print("\n—— Independent t-test (expNum-median 1s_veering, dd vs ld) ——")
...
# --- Write LaTeX macro definitions ---
macros_path = os.path.join(os.path.expanduser("../Statistics"), f"{plot_ID}_Grouped1sVeeringMacros.tex")
with open(macros_path, "w") as f:
    f.write(f"\\def\\GroupedVeeringDdDayMean{{{dd_mean:.3f}}}\n")
    f.write(f"\\def\\GroupedVeeringDdDaySD{{{dd_sd:.3f}}}\n")
    f.write(f"\\def\\GroupedVeeringLdDayMean{{{ld_mean:.3f}}}\n")
    f.write(f"\\def\\GroupedVeeringLdDaySD{{{ld_sd:.3f}}}\n")
    f.write(f"\\def\\GroupedVeeringDdDayVsLdDayTvalue{{{t_rel:.3f}}}\n")
    f.write(f"\\def\\GroupedVeeringDdDayVsLdDayPvalue{{{p_rel:.3e}}}\n")
    f.write(f"\\def\\GroupedVeeringDdDayVsLdDayEffectSize{{{abs(effect_size):.3f}}}\n")
