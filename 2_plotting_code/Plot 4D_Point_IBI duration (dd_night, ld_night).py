"""
Script: Plot 4D Median pre_IBI_time by Condition (Night)
Description: Computes and visualizes median pre-IBI time per bout under different lighting conditions (night data).
"""
#%%
...
def main():
    ...
    plot = (
        ...
        + labs(x="Condition",
               y="Median pre-IBI time (ms)",
               title="Nighttime Pre-IBI Interval by Condition")
        + theme_minimal()
        + theme(
    ...
    ibi_dd = combine_data.loc[combine_data[X_VAR] == "dd", Y_VAR].dropna()
    ibi_ld = combine_data.loc[combine_data[X_VAR] == "ld", Y_VAR].dropna()
    t1, p1 = stats.ttest_ind(ibi_dd, ibi_ld, nan_policy='omit')
...
def print_results(t1, p1, t2, p2):
    ...
    print("—— Independent samples t-test (epoch-level pre_IBI_time, dd vs ld) ——")
    print(f"t = {t1:.3f}, p = {p1:.3e}")
    print("\n—— Independent samples t-test (expNum-median pre_IBI_time, dd vs ld) ——")
...
def summary_stats(median_data, t_value, p_value, stats_dir):
    ...
    macros_path = os.path.join(stats_dir, f"{plot_ID}_GroupedPreIBITimeNightMacros.tex")

    macros = [
        f"\\def\\GroupedPreIBITimeNightDdMean{{{dd_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\GroupedPreIBITimeNightDdSD{{{dd_stats['SD'].values[0]:.3f}}}",
        f"\\def\\GroupedPreIBITimeNightLdMean{{{ld_stats['Mean'].values[0]:.3f}}}",
        f"\\def\\GroupedPreIBITimeNightLdSD{{{ld_stats['SD'].values[0]:.3f}}}",
        f"\\def\\GroupedPreIBITimeNightDdVsLdTvalue{{{t_value:.3f}}}",
        f"\\def\\GroupedPreIBITimeNightDdVsLdPvalue{{{p_value:.3e}}}",
        f"\\def\\GroupedPreIBITimeNightDdVsLdEffectSize{{{abs(effect_size):.3f}}}"
    ]
