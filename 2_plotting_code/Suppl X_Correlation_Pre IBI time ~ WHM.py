"""
Correlation pre_IBI_time vs WHM
Description: Computes and visualizes relationship between pre-IBI time and bout duration with binned medians and IQR.
"""
#%%
import os
import pandas as pd
...
def main():
    ...
    # Clip WHM values
    whm_lower, whm_upper = raw_data[X_VAR].quantile(WHM_CLIP).values
    ...
    plot = (
        ...
        + labs(x="bout duration (ms)",
               y="Pre-IBI time (s)",
               title="Pre-IBI Time vs bout duration")
        + theme_minimal()
        + theme(
