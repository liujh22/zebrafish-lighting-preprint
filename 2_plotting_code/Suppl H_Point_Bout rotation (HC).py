"""
Script: Plot Suppl H Median rot_total
Description: Computes and visualizes median rot_total per bout under different experimental conditions.
"""
#%%
import os
...
def main():
    ...
    # Print results
    # print("—— Independent samples t-test (epoch-level rot_total, ctrl vs cond) ——")
    # print(f"t = {t1:.3f}, p = {p1:.3e}")
    print("\n—— Independent samples t-test (expNum-median rot_total, ctrl vs cond) ——")
    ...
    macros_path = os.path.join(STATS_DIR, f"{plot_ID}_GroupedRotTotalMacros.tex")

    with open(macros_path, "w") as f:
        f.write(f"\\def\\GroupedRotTotalCtrlMean{{{ctrl_stats['Mean'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedRotTotalCtrlSD{{{ctrl_stats['SD'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedRotTotalCondMean{{{cond_stats['Mean'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedRotTotalCondSD{{{cond_stats['SD'].values[0]:.3f}}}\n")
        f.write(f"\\def\\GroupedRotTotalCtrlVsCondTvalue{{{t2:.3f}}}\n")
        f.write(f"\\def\\GroupedRotTotalCtrlVsCondPvalue{{{p2:.3e}}}\n")
        f.write(f"\\def\\GroupedRotTotalCtrlVsCondEffectSize{{{abs(effect_size):.3f}}}\n")
