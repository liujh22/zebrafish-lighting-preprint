...
def main():
    ...
    output_filename = f"Suppl C_scatter_{Y_VAR}_by_{X_VAR}_light.pdf"
    ...
    macros_path = os.path.join(STATS_DIR, "Table_GroupedScatterLightBoutRotationPostIBIMacros.tex")

    with open(macros_path, "w") as tex_file:
        for row in stats_results:
            grp = row['IBI_group'].replace(" ", "")
            tex_file.write(f"\\def\\GroupedScatterLightBoutRotationPostIBI{grp}AbsSlope{{{abs(row['Slope']):.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterLightBoutRotationPostIBI{grp}Slope{{{row['Slope']:.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterLightBoutRotationPostIBI{grp}Rsquare{{{row['Rsquare']:.3f}}}\n")
            tex_file.write(f"\\def\\GroupedScatterLightBoutRotationPostIBI{grp}Pvalue{{{row['pvalue']:.3e}}}\n")
