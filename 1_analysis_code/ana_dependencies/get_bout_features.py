...
import pandas as pd
from .plt_tools import round_half_up
import numpy as np
from .plt_tools import (day_night_split)
from .get_index import get_index
from scipy.signal import savgol_filter
import math

def get_bout_timeseries(root: str, FRAME_RATE: int, ztime, **kwargs):
        ...
        for subpath, subdir_list, subfile_list in os.walk(folder):
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    for i in day_night_split(bout_time, 'aligned_time', ztime=ztime).index:
    ...
    # If 'all', reclassify ztime
        if ztime == 'all':
            around_peak_data = day_night_split(around_peak_data, 'propBoutAligned_time', ztime='all')
        else:
            around_peak_data = around_peak_data.assign(ztime=ztime)

        all_around_peak_data = pd.concat([all_around_peak_data,
                                        around_peak_data.assign(cond0=cond0, cond1=cond1)])


    # Finally, add time column
    all_around_peak_data = all_around_peak_data.assign(time_ms=(all_around_peak_data['idx'] - peak_idx) / FRAME_RATE * 1000)

    #%
    ####################################
    # Calculate more parameters in dataframe #
    ####################################
    ...
    all_around_peak_data = all_around_peak_data.assign(
        heading_sub_pitch=all_around_peak_data['propBoutAligned_instHeading'] - all_around_peak_data[
    ...
    return all_around_peak_data, all_cond0, all_cond1, idxRANGE
...
def extract_bout_features_v5(bout_data: pd.DataFrame, peak_idx: int, FRAME_RATE: int, **kwargs):
    ...
    # Calculate time_ms
    bout_data = bout_data.assign(time_ms=(bout_data['idx'] - peak_idx) / FRAME_RATE * 1000)

    # ----- New Algorithm: Definition of an Auxiliary Function for Calculating FWHM ----- #
    def compute_fwhm_per_bout(df):
        # Ensure that the data within each bout is sorted in ascending order based on time_ms.
        df = df.sort_values('time_ms').reset_index(drop=True)
        speeds = df['propBoutAligned_speed'].values
        times = df['time_ms'].values

        # Locate local peaks and their indices
        ...
        # Traverse leftward to search for the "crossing" point:
        # Record the time when the value goes from below the half peak to being greater than or equal to the half peak.
        for i in range(peak_idx_local, 0, -1):
            if speeds[i] >= half_speed_local and speeds[i-1] < half_speed_local:
                left_time = times[i]
                break

        # Traverse to the right to search for the "crossing" point
   ...
   # --------------------------------------------------
    ...
    this_exp_features = pd.DataFrame(data={
        ...
        'spd_end': bout_data.loc[bout_data['idx'] == idx_end, 'propBoutAligned_speed'].values,
        'spd_peak': bout_data.loc[bout_data['idx'] == peak_idx, 'propBoutAligned_speed'].values,
        'angvel_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase), :].groupby('bout_num')[
            'propBoutAligned_angVel'].mean().values,
        'angvel_prep_phase': bout_data.loc[bout_data['idx'].isin(idx_prep_phase), :].groupby('bout_num')[
            'propBoutAligned_angVel'].mean().values,
        'angvel_post_phase': bout_data.loc[bout_data['idx'].isin(idx_post_phase), :].groupby('bout_num')[
            'propBoutAligned_angVel'].mean().values,
        ...
        'traj_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase), :].groupby('bout_num')[
            'propBoutAligned_instHeading'].mean().values,
        'spd_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase), :].groupby('bout_num')[
            'propBoutAligned_speed'].mean().values,

        'fish_length': bout_data.loc[bout_data['idx'].isin(idx_full_aligned), :].groupby('bout_num')[
            'fish_length'].mean().values
    })
    ...
    yy = (bout_data.loc[bout_data['idx'] == idx_post_bout, 'propBoutAligned_y'].values - bout_data.loc[
        bout_data['idx'] == idx_pre_bout, 'propBoutAligned_y'].values)
    xx = (bout_data.loc[bout_data['idx'] == idx_post_bout, 'propBoutAligned_x'].values - bout_data.loc[
        bout_data['idx'] == idx_pre_bout, 'propBoutAligned_x'].values)
    ...
    # ------ Calculate FWHM (Full Width at Half Maximum) using the new algorithm ------
    fwhm_result = bout_data.groupby('bout_num').apply(compute_fwhm_per_bout)
    ...
    # Step 2
    first_time_spd_larger_threshold = (
        bout_data[(bout_data['time_ms'] <= 0) & (bout_data['time_ms'] >= -250)]  # Filter data to include only time points between 0 and -250
        .sort_values(['bout_num', 'time_ms'], ascending=[True, False])  # Sort within each bout_num from 0 backward
        .groupby('bout_num')
        .apply(lambda group: group[group['propBoutAligned_speed'] <= THRESHOLD]['time_ms'].iloc[0] if any(
            group['propBoutAligned_speed'] <= THRESHOLD) else None)
    )

    # This gives a Series where each index is a 'bout_num' and each value is the first 'time_ms' that meets the threshold condition within that group.
    ...
    meanPitch_estimation = bout_data.query('idx in @idx_accel_phase').groupby('bout_num')[
        'propBoutAligned_pitch'].mean().values

    this_exp_features = this_exp_features.assign(
        rot_total=this_exp_features['pitch_end'] - this_exp_features['pitch_initial'],
        ...
        first_time_spd_larger_threshold=first_time_spd_larger_threshold)
        ...
        for subpath, subdir_list, subfile_list in os.walk(folder):
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    exp_path = os.path.join(subpath, exp)
                    # get pitch
                    exp_data = pd.read_hdf(f"{exp_path}/bout_data.h5",
                    ...
                    # assign frame number, total_aligned frames per bout
                    exp_data = exp_data.assign(
                        idx=round_half_up(len(exp_data) / total_aligned) * list(range(0, total_aligned)),
                        expNum=expNum,
                        exp=exp)
                    # - get the index of the rows in exp_data to keep (for each bout, there are range(0:51) frames. keep range(20:41) frames)
                    bout_time = pd.read_hdf(f"{exp_path}/bout_data.h5", key='prop_bout2').loc[:, 'aligned_time']
                    ...
                    for i in bout_time.index:
                        rows.extend(list(range(i * total_aligned + round_half_up(idxRANGE[0]),
                                               i * total_aligned + round_half_up(idxRANGE[1]))))

                    # assign bout numbers
                    trunc_exp_data = exp_data.loc[rows, :]
                    trunc_exp_data = trunc_exp_data.assign(
                        bout_num=trunc_exp_data.groupby(
                            np.arange(len(trunc_exp_data)) // (idxRANGE[1] - idxRANGE[0])).ngroup()
                    )
                    if not max_angvel_df.empty:
                        max_angvel_time = max_angvel_df.query("cond0 == @cond0 and cond1 == @cond1")[
                            'max_angvel_time'].item()
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    propBoutAligned_angVel = grp['propBoutAligned_pitch'].apply(
                        lambda grp_pitch: np.diff(savgol_filter(grp_pitch, 11, 3),
                                                  prepend=np.array([np.nan])) * FRAME_RATE,
                    )
                    ...
                    adj_angvel = selected_range['propBoutAligned_angVel_sm'] * (
                        np.repeat(adj_by_which, (idxRANGE[1] - idxRANGE[0])).values)
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    this_exp_features = this_exp_features.assign(
                        ...
                        exp = exp,
                        boxNum = attributes['boxNum'].values,
                    )
                    # day night split. also assign ztime column
                    this_ztime_exp_features = day_night_split(this_exp_features,'bout_time',ztime=which_zeitgeber, narrow_bin=if_narrow_bin)
