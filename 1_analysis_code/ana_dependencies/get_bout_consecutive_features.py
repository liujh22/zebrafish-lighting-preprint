...
def extract_consecutive_bout_features(connected_bout_df:pd.DataFrame, list_of_features:list, max_lag:int):
    ...
    for feature_toplt in list_of_features:
        ...
        grouped = connected_bout_df.groupby(['cond1','cond0','expNum','ztime'],group_keys=False)
        col_selected = feature_toplt
        for (cond1, cond0, expNum,ztime), group in grouped:
        ...
        long_df = long_df.sort_values(by=['cond0','cond1','expNum','ztime','id','lag'])
        if consecutive_bout_features.empty:
            consecutive_bout_features = long_df
        else:
            consecutive_bout_features = consecutive_bout_features.merge(long_df, on=['id', 'lag','cond1','cond0','expNum','ztime','exp_conduid'], how='left')
